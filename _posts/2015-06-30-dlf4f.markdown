...
Note furhter that there is now a `Scripts` folder, and when you change to that folder using the Linux `cd` command, you will find a file called `AdobeHDS.php`. The file ending tells you that the script is written in PHP, and this is the script that's going to do all the hard work for us!
...
3. In the network tab of the developer console, look for the manifest. You'll recognize it once you see it, because the response type is XML.
